...
SELECT f.title, COUNT(f_a.actor_id) AS 'Number of actors'
FROM film f
JOIN film_actor f_a
...
SELECT f.title, f.film_id, COUNT(f.film_id) AS 'Number of copies'
FROM film f
JOIN inventory i
...
AND title LIKE 'C%' OR title LIKE'K%';
...
SELECT f.title, f_c.category_id, cat.name AS 'category_name'
FROM film f
...
SELECT st.store_id, SUM(p.amount) AS 'Total Sales'
FROM payment p
JOIN rental r
...
SELECT st.store_id, c.city, ct.country
FROM store st
JOIN address a
